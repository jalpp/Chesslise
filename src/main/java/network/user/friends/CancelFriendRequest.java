...
public class CancelFriendRequest extends Request {
    ...
    // no one should cancel a friend but oh well sh*t happens
    public String cancelFriend(String discordid, String incommingFriendRequest) {
        ...
        this.getNetworkPlayers().updateOne(current,
                new Document("$pull", new Document("requestin", cancel.getString("id"))));
        this.getNetworkPlayers().updateOne(cancel,
                new Document("$pull", new Document("requestout", current.getString("id"))));

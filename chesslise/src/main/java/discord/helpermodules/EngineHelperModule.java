...
public class EngineHelperModule extends SettingSchemaModule implements CommandTrigger{
    ...
    public void sendwhiteSideMoveCommand() {
        try {
            ...
            if (board.isMated() || board.isDraw() || board.isStaleMate()) {
                ...
                gameHandler.updateFen(event.getUser().getId(),
    ...
    public void sendPlayEngine() {
        try {
            event.deferReply(true).queue();
            GameSchema schema = new GameSchema(event.getUser().getId(),
                    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",
                    Integer.parseInt(event.getOptionsByName("difficulty").get(0).getAsString()));
            gameHandler.createGame(schema);
            event.getHook()
                    .sendMessage("Game created against Stockfish Level "
                            + event.getOptionsByName("difficulty").get(0).getAsString()
                            + " \n run **/move <move>** use Chess SAN notation (e4) or UCI notation (e2e4)")
                    .queue();
    ...
    public void sendSetEngineMode() {
        try {
            event.deferReply(true).queue();
            gameHandler.updateDepth(event.getUser().getId(),
                    Integer.parseInt(event.getOptionsByName("difficulty").get(0).getAsString()));

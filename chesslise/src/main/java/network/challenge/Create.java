    ...
    public void create(SlashCommandInteractionEvent event) {
        ...
        if (getFinder().findConnected(event.getUser().getId())) {
            event.getHook().sendMessage(
                    "You must connect your account before creating a challenge! Run /connect to connect your account!")
                    .queue();
        } else {
            this.createChallenge(event.getUser().getId(), event.getUser().getName(),
                    getFinder().findPreferenceTc(event.getUser().getId()),
                    getFinder().findPreferencePl(event.getUser().getId()), "pending");
        }

        event.getHook().sendMessage("Challenge created! View status of your challenges by running /mychallenges")
                .queue();
    }

    public void createChallenge(String challengediscordid, String challengediscordusername, String tc, String pl,
            String status) {
